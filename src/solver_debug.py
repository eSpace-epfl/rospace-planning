...
class Solver:
    ...
    def plane_correction(self, chaser, chaser_next, target=None, save=False):
        """
            Correct plane inclination and raan with a single impulsive burn at the node.
        ...
        theta_i = 2.0 * np.pi + A_Li - chaser.kep.w

        if dO > np.pi or flag_O == -1.0:
            A_Li = np.pi + A_Li
            A_Lf = np.pi + A_Lf

        # BLf is the angular distance between O_f and the projection of c on the x-y plane
        ...
        print "Alpha ---->" + str(alpha*180.0/np.pi)
        print "ALi ----> " + str(A_Li*180.0/np.pi)
        print "ALf ----> " + str(A_Lf*180.0/np.pi)
        print "BLf ----> " + str(B_Lf*180.0/np.pi)

        psi = flag_O * np.arcsin(np.sin(A_Lf) * np.sin(i_f))
        if i_i > i_f:
            psi *= -1.0

        if (i_i > i_f > np.pi/2.0) or (np.pi/2.0 > i_f > i_i):
            phi = O_f + flag_O * B_Lf, flag_O
            print "ADDING"
        else:
            phi = O_f - flag_O * B_Lf
            print "SUBTRACTING", flag_O
        ...
        print "Theta_i ----> " + str(theta_i *180.0/np.pi)
        ...
        # Now it takes the closest point to do the burn, to decrease the total time of the mission.
        ...
        if (i_i > np.pi/2.0 and i_f > np.pi/2.0) or (i_i < np.pi/2.0 and i_f < np.pi/2.0):
            # Either both prograde or both retrograde
            if (i_f > i_i > np.pi/2.0) or (np.pi/2.0 > i_f > i_i):
                # Alpha stays alpha
                # c stay c CCW
                pass
            elif (i_i > np.pi/2.0 > i_f):
                alpha = np.pi - alpha
                # c stay c CCW
            elif (i_f > np.pi/2.0 > i_i):
                alpha = np.pi - alpha
                cx *= -1.0
                cy *= -1.0
                cz *= -1.0
            ...
            # One prograde and one retrograde
            if (i_i > i_f > np.pi/2.0) or (np.pi/2.0 > i_i > i_f):
                alpha = np.pi - alpha
                # c stay c CCW
            elif (i_f > np.pi/2.0 > i_i):
                # Alpha stay alpha
        ...
        print "Rotation vector: " + str(np.array([cx, cy, cz]))

        print theta_i * 180.0 / np.pi
        ...
        print "V_i: " + str(V_TEM_i)
        print "V_f: " + str(V_TEM_f)

        # Evaluate deltaV
        deltaV_C = V_TEM_f - V_TEM_i

        # Create command
        c = self._create_command(deltaV_C, theta_i, self.travel_time(chaser, chaser.kep.v, theta_i))
        ...
        # Apply command
        self._apply_command(chaser, target, c)
    ...
    def adjust_eccentricity_semimajoraxis(self, chaser, chaser_next, target):
        ...
        if abs(t) <= 1:
            ...
            c1 = self._create_command(deltaV_C_1, theta_1, self.travel_time(chaser, chaser.kep.v, theta_1))

            # Save single manoeuvre...
            self._save_result(chaser, target, len(self.command_line), True)

            # Apply the command
            self._apply_command(chaser, target, c1)

            c2 = self._create_command(deltaV_C_2, theta_2, np.pi * np.sqrt(a_int**3 / mu_earth))

            # Save single manoeuvre...
            self._save_result(chaser, target, len(self.command_line), True)

            # Apply the command
            self._apply_command(chaser, target, c2)
    ...
    def _propagator(self, chaser, target, dt, dv=np.array([0, 0, 0])):
        ...
        if target is not None:
            ...
            chaser.update_target_from_cartesian(r_C, v_C)
    ...
    def _create_command(self, deltaV, theta, duration):
        """ Given command information, create and add it to the command line.

        Args:
             deltaV:    Delta-V given in TEME reference frame.
             theta:     True anomaly where the burn should occur.
             duration:  Idle time -before- the command is executed (i.e the time needed to acquire the
                        exact burn position).
        ...
        c.duration = duration #self.travel_time(chaser, chaser.kep.v, theta_i)
    ...
    def _apply_command(self, chaser, target, command):

        self._propagator(chaser, target, command.duration)
        self._propagator(chaser, target, 1e-4, command.deltaV_C)